...
object Ex8 extends App  with Ex8Util{
  ...
  def recommendation(): Unit = {
    ...
    val (computedFeatures, computedThetas) = computeGradient(features, thetas, yWithNewRatings, rWithNewRatings, numOfUsers, numOfMovies, numOfFeatures, 0, 0.001f, 100)
    ...
    (0 until 10).map{
      i =>
        ...
        println(s" movie ${lastPredictions(index)}, ${movies(index)}")
    }
...
trait Ex8Util {
  ...
  def calcPDF(features: INDArray, mu: INDArray, sigma: INDArray): INDArray = {
    val det = sigma.getFloat(0,0)*sigma.getFloat(1,1) - sigma.getFloat(0,1)*sigma.getFloat(1,0)
  ...
  def computeF1(predictions: INDArray, labels: INDArray) : Float= {
    val P = if(predictions.sum(0).getFloat(0) > 0) {
      (0 until predictions.rows()).foldLeft(0) {
        case (p, index) =>
          val pred = predictions.getRow(index).getFloat(0)
          val label = labels.getRow(index).getFloat(0)
          if (label == 1.0 && label == pred)
            p + 1
          else
            p

      } / predictions.sum(0).getFloat(0)
    } else {
      0
    }

    val R = if(predictions.sum(0).getFloat(0) > 0) {
      (0 until predictions.rows()).foldLeft(0) {
        case (r, index) =>
          val pred = predictions.getRow(index).getFloat(0)
          val label = labels.getRow(index).getFloat(0)
  ...
  def selectThreshold(labelsVal: INDArray, predictions: INDArray) = {
    ...
    val epses: INDArray = Nd4j.linspace(predictions.min(0).getFloat(0),
      predictions.max(0).getFloat(0), steps)

    (0 until epses.columns()).foldLeft((0.0, 0.0)){
      case ((bestF1, bestEPs), i) =>
        val eps = epses.getColumn(i).getFloat(0)
  ...
  def computeCost(features: INDArray, thetas: INDArray, y: INDArray, r: INDArray, numOfUsers: Int, numOfMovies: Int, numOfFeatures: Int, lambda: Float = 0.0f) = {
  ...
  def computeGradient(features: INDArray, thetas: INDArray, y: INDArray, r: INDArray, numOfUsers: Int, numOfMovies: Int,
                      numOfFeatures: Int, lambda: Float = 0.0f, alpha: Float = 1 ,iters: Int = 1) :( INDArray, INDArray) = {
